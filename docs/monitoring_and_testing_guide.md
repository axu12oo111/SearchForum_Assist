...
#### 1. 统计系统 (`cogs/stats.py`)
- **Discord命令**: `/bot_stats`, `/server_stats`
- **功能**: 实时性能指标、缓存统计、系统资源监控
- **数据收集**: 自动收集命令使用频率、响应时间、错误率
...
```bash
# 安装系统监控工具
chmod +x scripts/system_monitoring_setup.sh
sudo ./scripts/system_monitoring_setup.sh
```
...
```bash
# 查看实时监控仪表板
/opt/discord-bot-monitoring/scripts/dashboard.sh

# 查看监控日志
tail -f /opt/discord-bot-monitoring/logs/resource_monitor.log

# 查看告警日志
tail -f /opt/discord-bot-monitoring/logs/alerts.log
```
